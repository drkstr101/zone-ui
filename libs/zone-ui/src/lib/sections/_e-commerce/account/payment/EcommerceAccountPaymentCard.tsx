...
export default function EcommerceAccountPaymentCard({ card }: Props) {
  const { value, label, cardNumber, cardHolder, expirationDate, isPrimary } =
    card;
    ...
    <>
        ...
        >
          <Stack direction="row" alignItems="center" flexGrow={1}>
            ...
            {isPrimary && (
              <Label
                color="info"
                startIcon={<Iconify icon="carbon:star-filled" />}
                sx={{ ml: 1 }}
              >
                Primary
              </Label>
      ...
      >
        <MenuItem disabled={isPrimary} onClick={handleClose}>
          <Iconify icon="carbon:star-filled" sx={{ mr: 1 }} /> Set primary
          payment
        </MenuItem>
